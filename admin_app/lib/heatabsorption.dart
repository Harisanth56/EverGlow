...
class _HeatAbsorptionState extends State<HeatAbsorption> {
  ...
  Future<void> fetchHeat() async {
    ...
    try {
      final response = await supabase.from('tbl_heatabsorption').select().order('heatabsorption_id', ascending: true);
      setState(() {
        _heat = response;
        _isLoading = false;
      });
  ...
  Future<void> insert() async {
    final text = heatController.text.trim();
    if (text.isEmpty) {
      _showSnackBar('Please enter a value', isError: true);
      return;
    }
    try {
      await supabase.from('tbl_heatabsorption').insert({'heatabsorption_name': text});
      _showSnackBar('"$text" Added Successfully');
      heatController.clear();
      fetchHeat();
    } catch (e) {
      _showSnackBar('Error adding entry', isError: true);
    }
  }
      ...
      children: [
        ...
        Column(
          ...
          children: [
            ...
            Text(
              "Define and manage technical absorption levels for product filtering",
        ...
        children: [
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text("ENTRY NAME", style: TextStyle(color: accentColor, fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1.2)),
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    prefixIcon: const Icon(Icons.thermostat_outlined, color: Colors.white24, size: 20),
                ...
                rows: _heat.asMap().entries.map((entry) {
                  ...
                  return DataRow(
                    cells: [
                      ...
                      DataCell(Text(data['heatabsorption_name'] ?? "", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w500, fontSize: 15))),
                      DataCell(Row(
                        children: [
                          ...
                          _buildActionButton(Icons.delete_outline, Colors.redAccent, () async {
                            await supabase.from('tbl_heatabsorption').delete().eq('heatabsorption_id', data['heatabsorption_id']);
                            fetchHeat();
                          }),
                        ],
                      )),
  ...
  void showEditDialog(Map<String, dynamic> data) {
    TextEditingController editController = TextEditingController(text: data['heatabsorption_name']);
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        backgroundColor: cardColor,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: const Text('Update Parameter', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
        content: Column(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              await supabase.from('tbl_heatabsorption').update({'heatabsorption_name': editController.text.trim()}).eq('heatabsorption_id', data['heatabsorption_id']);
